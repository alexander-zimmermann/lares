...
- **Backup Server** — configuration of the running PBS (one of the fleet VMs): OIDC realm, datastores, users, tokens and ACLs through its API, repositories, the subscription nag and user passwords over SSH; jobs and ACME follow as they move out of the first-boot script.
...
│   ├── 00-cluster/         #   PVE connection, ACME, backup jobs, PBS storage, users, OIDC realms
...
│   └── 60-pbs/             #   PBS connection, OIDC realms, host settings, datastores, users
